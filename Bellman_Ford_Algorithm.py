...
def bellman_ford(node,start): 
    distance = [ inf for i in range(len(node))]
    distance[start] = 0
    distance1 = copy.deepcopy(distance)
    for n in range(len(node)-1): # N-1번 iteration
        distance = copy.deepcopy(distance1)
        for i in range(len(node)) : # i node로 가는데, 
            if i == start :
                continue
            for j in range(len(node)): # j 거쳐서
                if distance[i] > distance[j]+ node[j][i]:
                    distance1[i] = distance[j]+node[j][i] # distance1에 임시 저장
    
    distance = copy.deepcopy(distance1)  # 복사
